...
#path_of_obj_pieces = r"E:\Virtual Drives\Tabletop Simulator\Tabletop Simulator\Mods\Models\New folder"
#obj_pieces_files = os.listdir(path_of_obj_pieces)
...
def generateObjFromVertices(verts, rot, wi, hi, nw, nh):
    ...
    for v in verts:
        ...
        file_text += "vt %s %s\n"%(fl(offset_x + v[0] * scale_x, 2 + int(math.ceil(math.log10(nw)-0.5))),fl(1 - (offset_y + v[1] * scale_y), 2 + int(math.ceil(math.log10(nh)-0.5))))
...
def generateFunkyPuzzle(width, height, POINT_COUNT):
    ...
    while piece_list.length() > 0:
        ...
        if rejig_bad_pieces and (p.is_self_intersecting() or p.get_closest_distance_to_self() < 0.07):
            #print("regen %dx%d %d %d"%(width,height,wi,hi))
...
def generateFiles(puzz_func, prefix):
    save_path = r"D:\Pieces"
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    with open(jn(save_path,"data.txt"),"w") as fclear:
        pass
    ...
    for lbl, dims_list in puzzles_to_do:
        for nw, nh in dims_list:
            ...
            folder_name = prefix + new_label.replace(":","x") + "-%dx%d"%(nw, nh)
            ...
            folder_path = jn(save_path, folder_name)
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)

            with open(jn(folder_path,"board.obj"),'w') as fout:
            ...
            for i, piece in enumerate(puzz):
                ...
                with open(jn(folder_path,"piece.%d.obj"%(i + 1)),'w') as fout:
                    fout.write(obj_text)
                    
            total_size= 0
            for f in os.listdir(folder_path):
                st = os.stat(jn(folder_path,f))
                total_size += st.st_size
            with open(jn(save_path,"data.txt"),"a") as fout:
                fout.write("%s width=%d height=%d size=%d seed=%d\n"%(folder_name,nw,nh,total_size,seed_inc))
            del puzz
        ...
        if event.type == pygame.QUIT:
            ...
            try:
                ...
                if do == 1:
                    ...
                    displayDecompFromFile(jn(path_of_obj_pieces,f))
                elif do == 2:
                    if isPyGameStarted:
                        gameDisplay.fill(black)
                    p = puzz.pop(0)
